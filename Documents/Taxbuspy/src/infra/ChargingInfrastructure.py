...
class PublicChargingInfrastructureOperator:
    ...
    def __init__(self, ch_op_id: int, public_charging_station_file: str, ch_operator_attributes: dict,
        ...
        def _loading_charging_stations(self, public_charging_station_file, dir_names) -> List[ChargingStation]:
            ...
            print(f"DEBUG: Proberen depotbestand te laden van: {public_charging_station_file}")
            try:
                stations_df = pd.read_csv(public_charging_station_file)
                print(f"DEBUG: Depotbestand succesvol geladen, aantal rijen: {len(stations_df)}")
                print(f"DEBUG: Eerste rijen:\n{stations_df.head()}")
            except FileNotFoundError:
                print(f"ERROR: Depotbestand niet gevonden: {public_charging_station_file}")
                raise
            except Exception as e:
                print(f"ERROR: Fout bij het laden van depotbestand: {e}")
                raise

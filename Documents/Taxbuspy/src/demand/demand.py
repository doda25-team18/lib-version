...
class Demand:
    ...
    # Voeg dit toe boven regel 94
        print("Debugging load_demand_file:")
        print("rq_file_dir:", rq_file_dir)  # Dit moet een string zijn
        print("rq_file_name:", rq_file_name)  # Dit moet ook een string zijn

        # Kijk of rq_file_dir of rq_file_name een dict is
        if isinstance(rq_file_dir, dict):
            print("ERROR: rq_file_dir is a dict, not a string!")
        if isinstance(rq_file_name, dict):
            print("ERROR: rq_file_name is a dict, not a string!")

        # De regel waar de fout optreedt
        abs_req_f = os.path.join(rq_file_dir, rq_file_name)

        print(f"Final file path: {abs_req_f}")  # Dit toont het samengestelde pad

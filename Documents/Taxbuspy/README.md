# FleetPy - Taxbuspy Simulation of On-Demand Rides in Helmond
**FleetPy** is a simulation framework to model and control the tasks of vehicle fleets (routing, user-assignment, charging, ...)

This repository contains a customized implementation of the FleetPy simulation framework, specifically configured to study the impact of service areas, detour time, and trip volumes on ride-pooling efficiency.
...
### C++ Router (Not needed for Taxbuspy)
...
## TAXBUSPY
This section describes how the input data for these specific simulations has been prepared and organized. A baseline scenario for April 13 has already been set up and is ready for immediate testing. By adjusting the configuration, the remaining data from the month of April can also be used.

All trips from the month of April have been matched and prepared for Taxbuspy. This processed data can be found in Taxbuspy/data/demand/Helmond_source/matched. From this folder, the data is further divided into subfolders based on the service area radius: 10km, 25km, and 50km. The original, unprocessed data can be found in Taxbuspy/data/demand/Helmond_source/raw.

The configuration scenarios used for running the simulations can be found in Taxbuspy/studies/Taxbus_test/scenarios.
...
<!-- ... (necessary modules for preprocessing can be installed by "pip3 install -r requirements_with_pp.txt") --> ## does not work
...
```
python run_examples.py ##for Fleetpy examples
python run_private_taxbuspy.py 4 ##for Taxbuspy example
```
#Fleetpy
...
#Taxbuspy
You should now have a directory containing a new simulated scenario. In each of the scenario directories, you will have a summary of the configuration, a simulation log file and several output files.
* Taxbuspy/studies/Taxbus_test/results
By default, a standard evaluation aggregating several results is performed after every simulation.
* standard_eval.csv

A folder named scripts has been created to store custom utility scripts that support the preprocessing and setup of the simulation. The following scripts are included:

afstand.py: Calculates the nearest network node for the depot (starting point).

build_network.py: Builds the transportation network based on OpenStreetMap (OSM) data.

convert_addresses_to_gps.py: Converts Dutch postal codes to GPS coordinates (longitude and latitude).

convert_timestamps.py: Converts date-time strings to seconds since January 1, 2024.

match_demand_data.py: Matches trip data to the nearest nodes in the network, preparing it for use in Taxbuspy.

trips.py: Script for generating random trips for testing and validation purposes.

These scripts are essential for preparing the input data and network configuration required by the Taxbuspy simulation environment.
